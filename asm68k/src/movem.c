		...
		if (status < ERROR) {
			...
			if (memOp.mode & DestModes) {
				...
				if (pass2) {
					...
					loc += 2;
					/* If the addressing mode is address register indirect with predecrement, reverse the bits in the register list mask */
...
char   *evalList(char *p,unsigned short *listPtr,int *errorPtr)
{
	...
	regList = 0;
	/* Check whether the register list is specified explicitly or as a register list symbol */
	if ((p[0] == 'A' || p[0] == 'D') && isRegNum(p[1]) && isTerm(p[2])) {
		/* Assume it's explicit */
		for (; ;) {			/* Loop will be exited via return */
				...
				if (p[2] == '/') {
					...
					if ((p[3] == 'A' || p[3] == 'D') && isRegNum(p[4]) && isTerm(p[5])) {
						...
						if (p[3] == 'A')
							...
							reg2 = p[4] - '0';
						/* Set all the bits corresponding to registers in the specified range */
						if (reg1 < reg2) {
							for (r = reg1; r <= reg2; r++)
								regList |= (1 << r);
						} else {
		...
		if (status < SEVERE) {
			/* The register list symbol must be previously defined in the program */
			if (status == UNDEFINED) {
				NEWERROR(*errorPtr, status);
