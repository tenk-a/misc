...
#define INTLIMIT	0xFFFFL
#define LONGLIMIT	0xFFFFFFFFUL
...
char   *eval(char *p,long *valuePtr,char *refPtr,int *errorPtr)
{
	...
	/* Assume that the expression is to be evaluated, at least until an undefined symbol is found */
	evaluate = TRUE;
...
*/
		...
		prec = precedence(*p);
		/* Do all stacked operations that are of higher precedence than the operator just examined. */
		...
		if (prec) {
			...
			/* If the character terminates the expression, then return the various results needed. */
			if (evaluate)
	...
	*refPtr = TRUE;
	if (*p == '-') {
		...
		*numberPtr = -x;
		...
		/* Evaluate unary plus operator recursively */
		p = evalNumber(++p, &x, refPtr, errorPtr);
		*numberPtr = x;
		return p;
	} else if (*p == '~') {
...
{
	/*	Compute the precedence of an operator.
		Higher numbers indicate higher precedence, e.g.,
		precedence('*') > precedence('+').
	    Any character which is not a binary operator will
	    be assigned a precedence of zero. */
	switch (op) {
	case '+':
	case '-':
		return 1;
...
{
	/* Performs the operation of the operator on the two operands. Returns OK or DIV_BY_ZERO. */
	switch (op) {
	case '+':
		*result = val1 + val2;
		return OK;
