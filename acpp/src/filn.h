...
int  Filn_Open(const char *name);						/* ソースファイルをオープンする */
char *Filn_Gets(void);									/* マクロ展開付１行入力. malloc したメモリを返すので、呼出側で開放のこと*/

void Filn_AddIncDir(char *dir);							/* (複数の)入力ディレクトリを設定する */
void Filn_GetFnameLine(char **s, int *line);			/* 現在入力中のファイル名と行番号を得る */
int  Filn_UndefLabel(const char *p);					/* マクロ名の削除. 登録されてなければなにもしない */
int  Filn_SetLabel(const char *name, const char *st);	/* マクロ名の登録. st=NULLならばnameを Cコンパイラの-Dname と同様の処理 */
int  Filn_GetLabel(const char *name, char **strp);		/* name がdefine されているか調べ、されていれば0以外を返す。*/
														/* そのとき *strpに定義文字列へのポインタを入れて返す. *strpの破壊は不可。*/

FILE *Filn_ErrReOpen(const char *name, FILE *fp);		/* エラー出力先fpをnameで再オープン. */
														/* name=NULLだと単にfpにし、fp=NULLだと新規にファイルへ出力 */
void Filn_ErrClose(void);								/* エラー出力先のclose */
int  Filn_Error(const char *fmt, ...);					/* ファイル名行番号付のエラー出力printf */
volatile int Filn_Exit(const char *fmt, ...);			/* Filn_Error と同じことをしたのちexit(1)する */
...
　・使い方。
	...
	・オプション設定は、入力を開始する前に済ますこと。入力中に変更した場合の
	  挙動は不明。
	...
	・最後は Filn_Term() を読んで終了。
	　ただし、現状の版では、取得したメモリの開放が不充分なので、
	　実質 Filn_Init/Filn_Termのペアは起動して一回だけしか使えない。ので、無茶をさせないこと。

　・注意
	・もともと汎用性をあまり考えていないルーチンを強引に作り変えたもので、
	　とくに、メモリは、使いっぱなし前提で作っていたので、現在もそのような
	　多少個所が残っている。
	・あまり使いこんでいない/使い方に偏りがある、ので、バグはいっぱい残っている
	　と思う。
	・&&や||で、左辺で結果が確定していても右辺を評価するしくみになっている。
	　このため、以下のような文があるとき、
		#if defined(M1) && M1(2,3) == 0
	もし、M1 が
		#define M1(a,b)	((a)*(b)+20)
	とでも定義されていれば大丈夫だが
	M1が未定義の場合、&&の右辺の解釈を誤ってエラーを出すかもしれない。
		#if defined(M1) && M1 == 0
	(M1が引数なしならば、強引にスキップしてつじつまはあうかもしれない。)
	（なお、#(式) 中では definedを使えないです……使えても使わないでください）。
*/
